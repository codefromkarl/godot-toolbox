...
# Assemble a new project with selected packs
...
- **12 architecture packs** — toolbox-owned autoload contracts for flow, data, save, rules, RPG, and more
...
### Optional Vendor Packs

| Pack | Plugin | Version | Requires | When to Enable |
|------|--------|---------|----------|----------------|
| `validation` | Godot Doctor | 2.1.2 | base | Scene/resource constraint CI validation |
| `debug` | Signal Lens | 1.4.1 | base | Signal-heavy projects needing visual flow debugging |
| `stateful` | Godot State Charts | 0.22.3 | base | Complex state machine architecture |
| `juice` | Sparkle Lite | 1.0.0 | base | Game-feel and feedback authoring |
| `automation` | GodotE2E | 1.1.0 | base | Runtime UI/E2E testing via pytest bridge |
| `input` | G.U.I.D.E | 0.12.0 | base | Cross-device input mapping and prompt icons |
...
| `ai-behavior` | Beehave | 2.9.2 | base | Behavior tree AI for NPCs/enemies |
| `save-state-lite` | SaveState Lite | 1.2.0 | base | Advanced save tooling and browser (**conflicts: save-core**) |
...
<details>
<summary><strong>完整中文说明 (Full Chinese Documentation)</strong></summary>
...
- `dialogue`：`Dialogue Manager`，显式选择时提供对话图/文本资源 authoring 和运行时 line playback 能力，默认不启用
...
- `packs/dialogue/`：`Dialogue Manager` 可选 pack，显式选择后提供对话图/文本资源 authoring 能力
- `packs/rpg-art-demo/`：RPG 美术/音频 demo 的非默认占位 pack，只提供 first-party text placeholders、NOTICE 和导入策略
...
- `Dialogue Manager` 作为显式 opt-in dialogue pack，不进入默认 bootstrap
- `gdterm` 不纳入工具箱
...
当前仓库没有 vendored RPG 美术/音频素材库。可选的 CC0/开源角色、怪物、tileset、战斗背景、UI skin、字体、SFX/BGM 来源见 `docs/rpg-art-asset-sources.md`；纳入前必须记录来源、版本、license/NOTICE 与本地目标路径。
