...
resolve_uv() {
  ...
  echo "[gdscript-toolkit-format] ERROR: uv is required." >&2
  exit 1
}
...
if [[ "${#targets[@]}" -eq 0 ]]; then
  echo "[gdscript-toolkit-format] No GDScript targets to format. Skipped."
  exit 0
fi

echo "[gdscript-toolkit-format] Running gdformat on ${#targets[@]} file(s)"
run_gdtool gdformat "${targets[@]}"

echo "[gdscript-toolkit-format] PASS"
